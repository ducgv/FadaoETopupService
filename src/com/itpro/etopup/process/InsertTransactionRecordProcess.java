 ...
 *
 */
public class InsertTransactionRecordProcess extends ProcessingThread {
	Queue queueInsertTransactionRecord = new Queue();
	...
	protected void process() {
		...
		if(!isConnected)
			return;
		TransactionRecord transactionRecord = (TransactionRecord) queueInsertTransactionRecord.dequeue();
		if(transactionRecord!=null){
			try {
				connection.insertTransactionRecord(transactionRecord);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				logError("Insert " + transactionRecord.toString()+ "; error:" + MySQLConnection.getSQLExceptionString(e));
				isConnected = false;
				queueInsertTransactionRecord.enqueue(transactionRecord);
			}
		}
