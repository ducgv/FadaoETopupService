 ...
 *
 */
public class UpdateBatchRechargeElementProcess extends ProcessingThread {
	Queue queueBatchRechargeElement = new Queue();
	...
	protected void process() {
		...
		if(!isConnected)
			return;
		BatchRechargeElement batchRechargeElement = (BatchRechargeElement) queueBatchRechargeElement.dequeue();
		if(batchRechargeElement!=null){
			try {
				connection.updateBatchRechargeElement(batchRechargeElement);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				logError("update " + batchRechargeElement.toString()+ "; error:" + MySQLConnection.getSQLExceptionString(e));
				isConnected = false;
				queueBatchRechargeElement.enqueue(batchRechargeElement);
			}
		}
